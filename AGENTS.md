...
7. **VÄITELOKIN YKSINOIKEUS (NO UNCLAIMED FINDINGS IN PROSE):** Mikään matemaattinen tai empiirinen "löydös", "todistettu"-väite tai absoluuttinen johtopäätös ei saa esiintyä missään dokumentaatiossa (edes brainstorm-, visio- tai suunnitelmapapereissa) ilman vastaavaa merkintää ja tilaa `MATH_CLAIMS.md`:ssä. Jos väitettä ei ole rekisteröity lokiin, kyseessä on vain ehdotus tai alustava hypoteesi, ja teksti on kirjoitettava ehdottomasti sen mukaisesti (esim. *"skanneri ei löytänyt ehdokasta tässä rajatussa avaruudessa"* eikä koskaan *"skanneri todisti"*).

8. **KIRJAAMISKIELI ON ENGLANTI (RECORDING LANGUAGE IS ENGLISH):** kaikki uusi dokumentaatio, koodikommentit, commit-viestit, moduulien tulosteet ja väitelokirivit kirjoitetaan **englanniksi**. Peruste on tutkimuksellinen eikä tyylillinen: alan kirjallisuus, sitaatit ja mahdolliset yhteistyökumppanit ovat englanninkielisiä, ja väite jota ulkopuolinen ei voi lukea ei ole tarkistettavissa. **Keskustelu ylläpitäjän kanssa käydään suomeksi** — sääntö koskee kirjattua jälkeä, ei vuorovaikutusta. **Migraatio:** aiemmin suomeksi kirjoitetut dokumentit käännetään sitä mukaa kun niitä muutenkin muokataan. `MATH_CLAIMS.md` käännetään **rivi kerrallaan silloin kun riviä muutenkin kosketaan, ei koskaan massakäännöksenä** — kalibroitu kielenkäyttö ("ei rikkomuksia välillä [a,b]") on juuri se mikä massakäännöksessä katoaa, ja se on väitelokin koko arvo. Sitaatit säilyvät aina alkukielellään.
