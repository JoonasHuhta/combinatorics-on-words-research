...
| **2. Positive control** (A3, k-abelian) | **Source opened (row 84); the control's target shape is now known. Not yet rebuilt or run.** | Row 84: Theorem 65's construction is a 5-uniform 4-to-3 morphism h2 applied ONCE to an existing abelian-square-free word over four letters (e.g. Keränen's g85), not a uniform ternary endomorphism's fixed point — confirming the flaw found empirically. **The control must be rebuilt to search that space** (apply h2, or a search over similar 4-to-3 morphisms, to a known 4-letter abelian-square-free source word, under 2-abelian equivalence) rather than uniform ternary morphisms k ≤ 6. **B16 repairs this at the root** — at S = all 9 a solution is known to exist, giving a validated end of the lattice |
