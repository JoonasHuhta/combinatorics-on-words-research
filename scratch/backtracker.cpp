...
bool extendWord(std::string& currentWord, int targetLength, const std::string& outputFile) {
    ...
    if (currentWord.length() > maxDepthReached) {
        ...
        std::ofstream logFile("progressive_log.txt", std::ios::app);
        if (logFile.is_open()) {
...
int main(int argc, char* argv[]) {
    ...
    // NOTE: Paths changed to relative paths for your workspace
    std::string dictionaryPath = "dictionary.txt"; 
    std::string outputPath     = "record_word_" + std::to_string(targetLength) + ".txt";
        
    validFactors.reserve(2403132); 
    std::cout << "Loading dictionary from " << dictionaryPath << "..." << std::endl;
    ...
    if (file.is_open()) {
        ...
        std::cerr << "You need to place Veikko's dictionary file in the root folder and name it 'dictionary.txt'." << std::endl;
        return 1;
    }
