 ...
 * g: {a..f} -> {a,b,c}^L applied to the fixed point h8^omega(a), asking
 ...
 * image g3(h8^omega(a)), and that image fails exactly on the open window
 ...
 * the complementary route: keep the base word h8^omega(a) fixed (its full
 ...
 * of 10-uniform maps (3^80) is CEGIS territory, but L = 1..3 is enumerable
 ...
 * --------------------------
 * The sweep is exhaustive over UNIFORM maps with |g(x)| = L for all eight
 ...
 * WHAT A RESULT MEANS
 * -------------------
 * A violation found in the image of the prefix h8^depth(a) is conclusive
 ...
 *
 * Usage:  node h8-image-sweep.js [--depth 7] [--L 1,2,3] [--budget 500000000] [--nosym]
 */

const { H8, G3, verifyMorphismIntegrity } = require('../src/morphisms.js');
...
function h8Prefix(depth) {
  let w = [4]; // start with 'e'
  for (let it = 0; it < depth; it++) {
    const next = new Array(w.length * 2); // max length 2
    let j = 0;
    for (let i = 0; i < w.length; i++) {
      const img = H8[EIGHT[w[i]]];
...
/** First-occurrence order of the eight letters in the base word. */
function firstOccurrenceOrder(x) {
  const first = new Array(8).fill(-1);
  for (let i = 0; i < x.length; i++) {
    if (first[x[i]] === -1) first[x[i]] = i;
  }
  if (first.includes(-1)) throw new Error('base prefix does not contain all eight letters; increase depth');
  const order = [0, 1, 2, 3, 4, 5, 6, 7].sort((p, q) => first[p] - first[q]);
  return { order, first };
}
...
function directScan(x, images, klo, khi) {
  let total = 0;
  for (let i = 0; i < 8; i++) total = Math.max(total, images[i].length);
 ...
 * LOSSLESS PRUNING, not a heuristic: every letter in the eight-letter alphabet
 ...
 * `h8-additive-image-sweep.js` uses for the additive analogue, row 77).
 */
...
function runControls_OLD() {
  ...
  // Control 1: first-occurrence order in h8^omega(a) is a,c,e,b,d,f at 0..5.
  const x2 = h8Prefix(2);
  const { order, first } = firstOccurrenceOrder(x2);
  const got = order.map(c => EIGHT[c]).join('');
  ...
  // Control 2: the 34-square census of g3(h8^6(a)) (same definition as test.js
  // test 5: distinct u|v with K in [1,5]) must reproduce exactly 34.
  const x6 = h8Prefix(6);
  let gw = '';
  for (let i = 0; i < x6.length; i++) gw += G3[EIGHT[x6[i]]];
  const n = gw.length;
 ...
 * independently coded full enumeration of all 3^8 = 6561 maps.
 */
function crossCheckL1(depth) {
  const x = h8Prefix(depth);
  ...
  for (let code = 0; code < 6561; code++) {
    const images = [];
    let c = code;
    for (let i = 0; i < 8; i++) { images.push([c % 3]); c = Math.floor(c / 3); }
...
function escalateSurvivor(imagesStr, depth) {
  ...
  const deeper = Math.min(depth + 2, 10);
  const xBig = h8Prefix(deeper);
...
function main() {
  ...
  const opt = { depth: 8, Ls: [1, 2], budget: 5e8, canonical: true, clean: false };
  ...
  console.log('=== h8-image-sweep: route (c), uniform images of h8^omega(a) ===\n');

  // const ctrl = runControls();
        
    const x = h8Prefix(opt.depth);
  console.log(`Base word: h8^${opt.depth}(a), ${x.length} letters. Test: abelian squares, K in [${KLO},${KHI}], scanned by end position.\n`);
  ...
  for (const L of opt.Ls) {
    ...
    console.log(`    latest first-violation at image symbol ${r.maxViolationPos}` +
      (r.bestImages ? ` (a:${r.bestImages[0]} b:${r.bestImages[1]} c:${r.bestImages[2]} d:${r.bestImages[3]} e:${r.bestImages[4]} f:${r.bestImages[5]} g:${r.bestImages[6]} h:${r.bestImages[7]})` : ''));
    console.log(`    work: ${r.symbolsAppended} symbols appended, ${dt}s`);
    for (const s of r.survivors) {
      console.log(`    SURVIVOR (bounded evidence only): a:${s[0]} b:${s[1]} c:${s[2]} d:${s[3]} e:${s[4]} f:${s[5]} g:${s[6]} h:${s[7]}`);
...
module.exports = {
  h8Prefix, firstOccurrenceOrder, directScan, canonicalize, sweepDFS,  
  isCleanBlock,  KLO, KHI
};
