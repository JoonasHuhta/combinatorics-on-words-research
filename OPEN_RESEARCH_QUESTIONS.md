...
- **The correctness question that must be settled first, before any implementation:** rows 30–31's ancestor closure was computed **for the pair (h₆, g₃)**, and the Proposition 5/6 bounds that make the box finite involve M_g. It is therefore **not** established that the same 116,578 templates apply to a different coding g. Either (a) derive a template set valid for all g with |g(x)| = L — using bounds that hold uniformly over that family — or (b) show the closure is g-independent. **Until one of these is done, the whole idea rests on a reuse assumption that is probably false.** This is the first task, and it is mathematics, not code.
